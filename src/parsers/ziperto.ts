...
/**
 * Source parser for www.ziperto.com.
 *
 * Behind Cloudflare (requiresJs: true), paginated catalog,
 * WordPress-style layout. Uses file hosts like 1fichier, MegaUp,
 * SendCM, DooDrive. The parser targets entry-title links in the
...
export const zipertoParser: SourceParser = {
   ...
   * avoids sidebar, navigation, and ad links.
   ...
   * Extracts the game name from <h1> (post title) or <title>,
   * then finds all external links in the .entry-content download
   * section. Avoids sidebar, widget, and ad links.
   *
   * Returns raw URLs — the orchestrator filters through the
   * file host registry and intermediary filter.
   */
  extractDownloadLinks(html: string, detailPageUrl: string): {
    ...
    searchArea.find('a[href]').each((_, el) => {
      ...
      // Skip links in sidebar, widget areas, or ad-related containers
